...
If this is your **first** apply, expect **15 to add, 0 to change, 0 to destroy** — or
**17** for a ceremony release, where the two extra are the `secretVersionAdder`
bindings. Your plan also shows a `provenance_verified` output; that is the proof the
provenance check ran.
...
edited, so the binding is replaced. A changed **keygen** digest shows as 1 change. Those
destroys are expected. See *Apply a later release* in `PARTNER_GUIDE.md`.
